document.addEventListener('DOMContentLoaded', () => {
    ...
    const albumInput          = document.getElementById('editor-album');
    ...
    const storageKey = itemID ? `editor:song:${itemID}` : 'editor:song:new';
    ...
    const lookup = new LookupController({ endpoint: '/songs/lookup', excludeID: parseInt(idInput?.value) || null });
    ...
    function setState(state) {
        ...
        if (typeof state.album === 'string' && !albumInput.value) albumInput.value = state.album;
    ...
    function applyMetadata(data) {
        ...
        if (!artistInput.value && data.artist) artistInput.value = data.artist;
    ...
    function fillPreview() {
        ...
        set('preview-artist',       artistInput?.value || '');
        set('preview-album',        albumInput?.value || '');
    ...
    artistInput?.addEventListener('input', saveDraft);
    albumInput?.addEventListener('input', saveDraft);
    ...
    const imagePicker = new ImagePickerController(
        ...
        { onSelectImage: (id, url) => artwork.setArtwork(id, url), onInsertMarkdown: (md) => notes.insertMarkdown(md), imageTitle: 'Select album artwork' }
    );
    imagePicker.init();
