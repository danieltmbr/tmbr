...
public struct Permission<Input>: Sendable {
    ...
    public struct User: Sendable {
        public let user: CoreAuth.User
        
        public let userID: UserID
        
        public init(user: CoreAuth.User, userID: UserID) {
            self.user = user
            self.userID = userID
        }
        ...
        public subscript<V>(dynamicMember keyPath: KeyPath<CoreAuth.User, V>) -> V {
    ...
    public init(
        ...
        self.init { (request, input) in
            let user = request.auth.get(CoreAuth.User.self)
