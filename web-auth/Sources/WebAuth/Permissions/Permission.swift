...
public struct Permission<Input>: Sendable {
    ...
    public struct User: Sendable {
        public let user: WebAuth.User
        
        public let userID: UserID
        
        public init(user: WebAuth.User, userID: UserID) {
            self.user = user
            self.userID = userID
        }
        ...
        public subscript<V>(dynamicMember keyPath: KeyPath<WebAuth.User, V>) -> V {
    ...
    public init(
        ...
        self.init { (request, input) in
            let user = request.auth.get(WebAuth.User.self)
